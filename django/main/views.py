...
def handle_request(request):
	...
	handled = {}
	handled['results'] = True
	handled['result_image_path'] = None

	fields = ['image_title', 'num_of_results', 'hm_lvl', 'certainty']
	...
	results = run(settings.MEDIA_ROOT, filename, int(handled['num_of_results']), int(handled['hm_lvl']), int(handled['certainty']))
	...
	if handled['result_features']: #not None
		if len(handled['result_features'])>0: #not an empty list
			handled['best_feature'] = handled['result_features'][0]
			print ("Best feature: " + handled['best_feature'])
